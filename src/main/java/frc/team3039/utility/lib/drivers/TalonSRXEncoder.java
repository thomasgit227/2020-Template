...
public class TalonSRXEncoder extends WPI_TalonSRX {
	...
	public TalonSRXEncoder(int deviceNumber, double encoderTicksToWorld, boolean isRight,
			FeedbackDevice feedbackDevice) {
		super(deviceNumber);
		final ErrorCode sensorPresent = this.configSelectedFeedbackSensor(feedbackDevice, PID_IDX, TIMEOUT_MS);
		if (sensorPresent != ErrorCode.OK) {
			DriverStation.reportError(
					"Could not detect encoder for device ID = " + deviceNumber + " : " + sensorPresent, false);
		}
